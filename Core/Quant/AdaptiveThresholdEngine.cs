...
namespace AutoInvest.Core.Quant
{
    ...
    public static class AdaptiveThresholdEngine
    {
        ...
        {
            // 기본값 가져오기 (0.65)
            decimal defaultThreshold = 0.65m;
            if (decimal.TryParse(AppConfigManager.Get("BUY_THRESHOLD", "0.65"), out decimal configVal))
            {
                defaultThreshold = configVal;
            }

            try
            {
                // 최근 100건의 이력 가져오기
                List<decimal> history = MarketSnapshotDAO.GetHistoricalProbabilities(ticker, 100);
            ...
            {
                Logger.Error($"[AdaptiveThreshold] 임계값 산출 실패 ({ticker}): {ex.Message}");
