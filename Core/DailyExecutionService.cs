    ...
    public class DailyExecutionService
    {
        ...
        public async Task<string> RunDailyCycleAsync()
        {
            ...
            if (!client.IsLoggedIn)
            {
                ...
                if (!loginOk)
                {
                    string err = "로그인 실패 — 주문 스킵";
                    Logger.Error($"[DailyCycle] {err}");
                    return err;
                }
            }
            ...
            {
                string warn = "전략 데이터 없음 — 주문 스킵";
                Logger.Warn($"[DailyCycle] {warn}");
                return warn;
            }
            ...
            // 4. AI 성과 과거 기록 평가 (7일 전 데이터)
            var evaluatedItems = await EvaluatePastAiPerformanceAsync(client);

            // 5. 일일 운용 보고서 (이메일 발송)
            await SendDailyReportAsync(results, evaluatedItems);

            Logger.Info("[DailyCycle] ✔ 일일 매매 사이클이 무사히 종료되었습니다.");
            return $"사이클 완료: {summary}";
        }
        ...
        private async Task SendDailyReportAsync(List<SmartOrderResult> todayResults, List<AiPerformanceDto> evaluatedItems)
        {
            try
            {
                ...
                string ordersHtml = todayResults.Count == 0 ? "<p>오늘 발생한 매매 신호가 없어서 아무 종목도 사거나 팔지 않았습니다.</p>" : "<ul>";
