...
- **신호/임계값/합의 스코어링/AI 분석/적응형 임계값/리밸런싱 로직을 다시 추가하지 말 것.**
  새 기능은 "사람이 지정한 종목별 고정 수량을 그대로 매수"라는 기계적 적립 원칙을 깨지 않아야 함.
...
- 종목별 매수 수량·예산은 `DcaSettings`(DB 우선 → appsettings 폴백)를 통해서만 읽고 쓴다.
...
- 배분 로직(`PlanPurchases`)은 순수 함수이므로 입력/기대출력 시나리오로 단위 검증한다(예: 지정 수량 그대로 매수, 현재가 없는 종목 제외, 총 매수금액 합산, 수량 0 제외).