...
| 구분 | 항목 |
|------|------|
| 필수 (값은 아무거나 새로 만들어도 됨) | `MASTER_KEY` — 2026-08-06에 `TB_APP_CONFIG`의 KIS 암호문 사본을 삭제해 **복호화할 대상이 없습니다**. 그래도 없으면 기동이 거부되므로(`Program.cs`) 환경변수는 반드시 유지하고, 잃었다면 base64 32바이트를 새로 만들어 넣으면 됩니다 |
...
- 값을 잃어 곤란한 항목은 이제 없습니다. `MASTER_KEY`조차 복호화할 대상이 없어 새로 만들어 넣으면 되고, 나머지는 원본이 외부(KIS·증권사·Resend)에 있습니다. 다만 `MASTER_KEY`가 **비어 있으면 기동이 거부**되므로 값의 존재 자체는 필수입니다.
