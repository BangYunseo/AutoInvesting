...
2. **`Core/DailyExecutionService.cs`**
   - **핵심 포인트**: `RunDcaCycleAsync()` 단 하나의 흐름만 가집니다. **월 1회 멱등 가드**(`TB_APP_CONFIG`의 `DCA_LAST_RUN_MONTH`, KST 기준) 확인 → 당월 이미 적립했으면 로그인 전에 스킵 → 로그인 → `DcaSettings.Load()`로 **현재 월에 적용할 종목별 수량·예산 로드** → `DcaAccumulationEngine.AccumulateAsync()` 실행 → 체결 1건 이상이면 당월 마커 저장 → 결과 이메일 보고서 발송.
   - **주목할 코드**: 크론이 매일 호출돼도 **당월 1회만 집행**되게 하는 멱등 가드(실거래 과매수 방지의 핵심). 그리고 이미 적립 완료·로그인 실패·이번 달 매수할 수량 없음 같은 조기 종료 시에도 항상 보고서를 보내는 부분. 판단·타이밍 로직이 전혀 없다는 점을 확인하세요.
