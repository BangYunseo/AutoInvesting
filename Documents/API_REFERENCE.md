...
일일 전체 사이클(퀀트 단독 매매·리밸런싱·메일 리포트)을 **백그라운드로 실행**합니다. (AI 평가는 현재 휴면) 외부 스케줄러에서 1일 1회 호출하는 용도이며(운영 환경에서는 GitHub Actions 워크플로우 `.github/workflows/daily-run.yml`가 매일 KST 23:25에 호출), 타임아웃 방지를 위해 즉시 `202` 를 반환합니다.
