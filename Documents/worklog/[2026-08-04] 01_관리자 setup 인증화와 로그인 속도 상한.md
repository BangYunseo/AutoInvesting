...
같은 리뷰에서 나온 **Swagger 프로덕션 익명 공개**도 함께 닫았다. 전역 필터는 MVC 액션 필터라 미들웨어인 `UseSwagger`에는 걸리지 않고 환경 분기도 없어, 외부 curl로 200을 받은 기록이 사전 점검에서 이미 확인됐다. 인증 우회는 아니지만 `ManualOrderRequest`의 `acknowledgeTax`와 `dca-run?force=true`처럼 UI가 쓰지 않는 우회 수단까지 스키마로 광고했다. `app.Environment.IsDevelopment()` 분기로 감쌌으므로 Render(기본 `Production`)에서는 켜지지 않는다.
