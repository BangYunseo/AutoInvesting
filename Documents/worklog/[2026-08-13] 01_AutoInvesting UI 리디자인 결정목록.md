...
> **아래 A~I 절은 `[2026-08-13] 03_UI 디자인 정의 항목.md`로 대체되었다.** 이 구조는 일반 웹 UI 프레임에서 가져온 것이라 이 코드베이스의 실제 구성과 어긋나고(위 `### 용어 정의 불일치` 참조), 동시에 투자 도구라서 반드시 정해야 하는 항목이 빠져 있다. 정의는 03에서 진행하고, A~I는 대조용으로만 남긴다. 위쪽 `[조사]` 결과와 `### 커스터마이징 계층`은 그대로 유효하다.
