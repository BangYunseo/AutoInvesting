...
| 항목 | 설명 |
|------|------|
| **목표비중** | 종목별 목표 비율 (예: `SPLG 40% / QQQM 30% / SCHD 20% / GLD 10%`) |
| **비중 편집(UI)** | 적립 설정 페이지에서 **100% 기준(%)**으로 입력. **ETF 개수**를 넣으면 100%를 자동 균등 분배하고, 각 종목이 예산에서 차지하는 비중·금액을 실시간 표시 |
| **비중 상한** | 비중 합계는 **100%를 초과할 수 없음**. 투자 규모를 키우려면 비중이 아니라 **월 예산**을 조정 |
...
| **순수 함수** | 배분 계산(`PlanPurchases`)은 외부 I/O 없는 순수 함수로 분리되어 단위 검증 가능 (저장 시 %→분수 변환) |
...
- [x] 적립 설정 UI 개선 — 비중 100% 기준(%) 편집, ETF 개수 자동 균등 분배, 비중 합계 100% 상한, 예산 기준 종목별 금액 표시
